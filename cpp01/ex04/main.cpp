...
/*   Updated: 2023/12/13 20:59:20 by samusanc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void findAndReplace(std::string& str, const std::string& toFind, const std::string& toReplace) {
	...
	if (argc != 4)
		...
		if (outFile.is_open()) {
			...
			if (inFile.is_open()) {
				...
				std::cerr << "file can not be open, pls try different name" << std::endl;
			outFile.close();
			return 1;
		} else {
			std::cerr << "file can not be created, pls try different name" << std::endl;
			return 1;
		}
	}
}
